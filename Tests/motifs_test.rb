...
def motif_mot_complet( regex )
  /\b#{Motifs.const_get(regex)}\b/
end

def motif_ligne_complete( regex )
  /^#{Motifs.const_get(regex)}$/
end

describe Motifs do
  describe Motifs::NUM_VIN do
  ...
  describe Motifs::CHAINE do
    ...
    it_ "matche pas si le separateur est present", :intermediaire do
      "Cotes-du-Rhone:Rasteau".wont_match motif_ligne_complete( :APPELLATION )
    end
  end

  describe Motifs::NOTE do
    it_ "matche un nombre compris entre NOTE_MIN et NOTE_MAX", :intermediaire do
      (Motifs::NOTE_MIN..Motifs::NOTE_MAX).each do |i|
    ...
    it_ "ne matche rien d'autre", :intermediaire do
      ...
      "#{Motifs::NOTE_MAX+1}".wont_match motif_mot_complet( :NOTE )
