...
describe Vin do
  ...
  let(:chianti) { Vin.new(10, date_achat, :rouge, "Chianti Classico", 2011, "Fontodi", 20.99) }

  let(:chianti_note) { Vin.new(11, date_achat, :rouge, "Chianti Classico",  2011, "Fontodi", 20.99,
                               4, "Tres bon!") }
  ...
  describe ".creer" do
    it_ "cree un vin avec nouveau numero qui n'est pas bu" do
      Vin.instance_variable_set(:@numero_max, 10) # Plus direct... meme si bizarre!
      date_achat = Time.new(2015, 10, 12).to_date

      vin = nil
      Time.stub :now, date_achat do # Pour avoir la date appropriee.
        vin = Vin.creer( :rouge, "Chianti", 2012, "Fontodi", 21.99 )
      end
  ...
  describe "#<=> --- tests intermediaire" do
    context "avec des comparateurs explicites et possiblement multiples" do
      let(:v1) { Vin.new(10, date_achat, :rouge, "Chianti", 2011, "Fontodi", 20.99) }
      let(:v2) { Vin.new(11, date_achat, :rouge, "Chianti Classico", 2012, "Fontodi", 19.99) }

      before { @comparateurs = Vin.comparateurs }
      after  { Vin.comparateurs = @comparateurs }
      ...
      it_ "produit un resultat qui depend du nombre de comparateurs", :intermediaire do
        ...
        Vin.comparateurs = [:nom, :millesime]
        assert v1 < v2
      end

      it_ "traite deux comparateurs et le resultat depend de l'ordre", :intermediaire do
        Vin.comparateurs = [:millesime, :prix]
        assert v1 < v2

        Vin.comparateurs = [:prix, :millesime]
        assert v1 > v2
      end

      it_ "traite trois comparateurs", :intermediaire do
        Vin.comparateurs = [:prix, :appellation, :nom]
        assert v1 > v2
      ...
      it_ "traite quatre comparateurs", :intermediaire do
        Vin.comparateurs = [:nom, :appellation, :date_achat, :numero]
        assert v1 < v2
      end
    end
  end
  ...
  describe "#to_s -- tests_intermediaire" do
    ...
    it_ "traite les justifications et la largeur maximum", :intermediaire do
      chianti.to_s( "%4I:%-4I:%.4I" ).must_equal '  10:10  :10'
    end
